...
namespace CN100.EnterprisePlatform.ORM.DB
{
    public class OrlTable : ITable
    {
        ...
        public OrlTable(Type type, string name, string schema)
        {
            this.type = type;
            if (name != null)
        ...
        protected virtual string GetInsertSql()
        {
            if (insertSQL == null)
            {
                ...
                if (identity != null)
                {
                    sql.Append("id:=").Append(identity.Name).Append("CURRVAL ");
                }
            ...
            if (updateSQL == null)
            {
                ...
                foreach (SqlColumn column in columns)
                {
                    if (column.IsReadable && !column.IsPK && !column.IsID)
                    {
                        if (comma)
                            ...
                            comma = true;
                        sql.Append(column.Name).Append("=:").Append(column.Ordinal);
                    }
                }
                sql.Append(" where ");
                comma = false;
                foreach (SqlColumn column in columns)
                {
                    if (column.IsReadable && column.IsPK)
                    {
                        if (comma)
                            sql.Append(" and ");
                        sql.Append(column.Name).Append("=:pk").Append(column.Ordinal);
                    }
                }
                //sql.Append(";");
        ...
        protected virtual string GetDeleteSql()
        {
            if (deleteSQL == null)
            {
                ...
                if (schema != null && schema.Length > 0)
                    sql.Append(schema).Append(".");
                sql.Append(name).Append(" where 1=1");
                foreach (SqlColumn column in columns)
                {
                    if (column.IsPK)
        ...
        {
            StringBuilder buf = new StringBuilder("EXECUTE IMMEDIATE ");
            buf.Append(proc);
            if (nParams > 0)
                buf.Append(" ");
