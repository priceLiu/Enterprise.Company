...
{
    public class LitTableFactory
    {
        public static readonly LitTableFactory Instance = new LitTableFactory();

        private LRUCache<LitTable> cache = new LRUCache<LitTable>(50);

        private LitTableFactory()
        { }

        public LitTable Build(Type type)
        {
            LitTable table = cache.Get(type.FullName);
            if (table == null)
            {
